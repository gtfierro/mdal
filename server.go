...
func getTLS(host, cacheDir string) (*tls.Config, error) {
	manager := autocert.Manager{
		...
		Email:      "gt.fierro@cs.berkeley.edu",
	}
...
func NewServer(c *Core, laddr string) *Server {
	...
	tlscert, _ := getTLS("corbusier.cs.berkeley.edu:8088", "certs")
	creds := credentials.NewTLS(tlscert)

	_ = creds
	// max 128mb messages
	grpcServer := grpc.NewServer(grpc.MaxSendMsgSize(1024*1024*64), grpc.MaxRecvMsgSize(1024*1024*64), grpc.RPCCompressor(grpc.NewGZIPCompressor())) //, grpc.Creds(creds))
	srv := &Server{
		grpcsrv: grpcServer,
		core:    c,
	}
	...
	if err := mdalgrpc.RegisterMDALHandlerFromEndpoint(context.Background(), mux, ":8088", opts); err != nil {
		log.Error(err)
		panic(err)
	}
	httpmux := http.NewServeMux()
	httpmux.Handle("/", mux)
	//if err := proto.RegisterMDALServer
	go grpcServer.Serve(l)
	go http.ListenAndServe(":8089", corsc.Handler(mux))
...
func (srv *Server) DataQuery(req *mdalgrpc.DataQueryRequest, grpcsrv mdalgrpc.MDAL_DataQueryServer) error {
	...
	//log.Warning(respnew.Values[0])
	resp.Mapping = make(map[string]*mdalgrpc.DataQueryResponse_VarMap)
	//resp.Context
	for _, vardec := range query.Variables {
		...
		for _, uid := range vardec.uuids {
			...
			u = append(u, bytes)
			resp.Context = append(resp.Context, &mdalgrpc.DataQueryResponse_Row{
				Uuid: uid.String(),
				Row:  vardec.Context[uid.String()].ToMapSS(),
			})
		}
		resp.Mapping[vardec.Name] = &mdalgrpc.DataQueryResponse_VarMap{Uuids: u}
	}
...
func (srv *Server) DataQuery2(req *mdalgrpc.DataQueryRequest, grpcsrv mdalgrpc.MDAL_DataQuery2Server) error {
	log.Warning("ALTERNATE BETA QUERY")
	fmt.Printf("%+v\n", req)
	var query Query
	var resp mdalgrpc.DataQueryResponse
	ctx := grpcsrv.Context()
	...
	// run query
	_, respnew, err := srv.core.HandleQuery2(ctx, &query)
	if err != nil {
		resp.Error = errors.Wrap(err, "Could not run query").Error()
		log.Error(resp.Error)
		return grpcsrv.Send(&resp)
