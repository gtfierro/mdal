...
type btrdbClient struct {
	...
	queries2    chan dataRequest
	workerpool  chan chan dataRequest
	workerpool2 chan chan dataRequest
}
...
func connectBTrDB() *btrdbClient {
	b := &btrdbClient{
		...
		queries2:    make(chan dataRequest),
		workerpool:  make(chan chan dataRequest, maxWorkers),
		workerpool2: make(chan chan dataRequest, maxWorkers),
	}
	...
	go func() {
		for {
			...
			case request := <-b.queries2:
				go func(request dataRequest) {
					worker := <-b.workerpool2
					worker <- request
				}(request)
			}
		}
...
func (b *btrdbClient) handleRequest2(req dataRequest) error {
	...
	defer req.done()
	if req.time.WindowSize > 0 {
		if err := b.getWindow2(req); err != nil {
			return err
		}
	} else if err := b.getData(req); err != nil { // raw
		return err
...
func (b *btrdbClient) DoQuery2(ctx context.Context, q Query) (*Timeseries, *mdalgrpc.DataQueryResponse, error) {
...
func (b *btrdbClient) getWindow2(req dataRequest) error {
	span, ctx := opentracing.StartSpanFromContext(req.ctx, "WindowQuery")
	defer span.Finish()
	stream, units, err := b.getStream(req.uuid)
	if err != nil {
	...
	//arr := b.NewFloat64Array()
	log.Warning(bldr.count(), len(bldr.value))
	bldr.build(req.resp)
	bspan.Finish()
	retrieveDuration := time.Since(retrieveStart)
